...
---------------------------------------------------------------------------------
---------------------------------------------------------------------------------
Revised by: Pai Li (lipai@mail.ustc.edu.cn) for the constant current mode
Date of last version: 2022/07/08
---------------------------------------------------------------------------------
'''
...
def find_lp(n2,ac,denij,n_max):
    dmax=denij.max()
    #print("dmax,n_max ",dmax,n_max)
    if(dmax<averaged_current):
        print("dmax<averaged_current!!!")
        exit()
    #print("init n2: ",n2)
    while denij[n2]<denij[n2+1]:
        n2=n2+1
        if(n2==n_max):
            print("n2==n_max!!!")
            exit()
    #print("changed n2: ",n2)
    #print("d[n2],d[n2+2]: ",denij[n2],denij[n2+1])
    ...
    if(denij[n2]<ac):
        while denij[n2]<ac:
            ...
            if(n2==0):
                print("n2==0!!!")
                exit()
        return n2+(denij[n2]-ac)/(denij[n2]-denij[n2+1])
    elif(denij[n2+1]>ac):
        while denij[n2+1]>ac:
            n2=n2+1
            if(n2==n_max):
...
for h in heights:
    ...
    # Contant current mode.
        ...
        print("averaged current: ",averaged_current)
        given_current=input('Enter your setting current (if negative, the averaged current will be used):')
        given_current=float(given_current)
        if(given_current>0):
            averaged_current=given_current
        c1 = density[:,:,n2]
    ...
    for i in range(supercell_xngridpoints):
        for j in range(supercell_yngridpoints):
            ...
            if scan_mode_option == "1":
                ...
                lp=find_lp(n2=n2,ac=averaged_current,denij=density[mi,nj,:],n_max=ngridpoints[2])
                #print("lp: ",mi,nj,lp)
                H[i][j] = lp*cell_lengths[2]/ngridpoints[2]
